...
const Doctors = () => {
  ...
  return (
    ...
    >
      <p className="text-[var(--foreground)]">Browse through the doctors specialist.</p>
      <div className="flex flex-col sm:flex-row items-start gap-5 mt-5">
        <button
          className={`${
            showFilter ? "bg-[var(--primary)] text-white" : ""
          } py-1 px-3 border rounded text-sm transition-all sm:hidden cursor-pointer`}
        ...
        <div
          ...
          } flex-col gap-4 text-sm text-[var(--foreground)]`}
        >
          {specialityList.map((doc) => (
            <p
              ...
              className={`w-[94vw] sm:w-auto pl-3 py-1.5 pr-16 border border-[var(--border)] rounded transition-all cursor-pointer ${
                slug === doc.slug
